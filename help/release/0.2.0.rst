...
* :module:`YCMEPHelper`: variables in commands are no longer replaced. Instead
  ...
  are automatically replaced by ExternalProject. The additional
  ``<YCM_EP_CMAKE_ARGS>`` tag is is replaced with arguments that
  :module:`YCMEPHelper` would pass to the CMake configure command. Using this
  it is possible to change the CMake command, but still pass the variables
  required for a superbuild.
